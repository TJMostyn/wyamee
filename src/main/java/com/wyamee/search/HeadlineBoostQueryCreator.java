...
public class HeadlineBoostQueryCreator extends AbstractQueryCreator {
	...
	public Query generate(List<NGramScore> ngramScores) {
		...
		for (NGramScore ngramScore : ngramScores) {
			try {
				Query headlineQuery = createQuery(ngramScore.getNgram() + "^" + HEADLINE_BOOST, DocumentFields.HEADLINE);
				queryBuilder.add(headlineQuery, BooleanClause.Occur.SHOULD);
				queryBuilder.add(
